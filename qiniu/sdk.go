...
func Upload(key string, localFile string) string {
	putPolicy := storage.PutPolicy{
		Scope: setting.AppSetting.QBoxBucket,
	}
	mac := qbox.NewMac(setting.AppSetting.QBoxAccess, setting.AppSetting.QBoxSecret)
	upToken := putPolicy.UploadToken(mac)
	cfg := storage.Config{}
	// 空间对应的机房
	...
	return setting.AppSetting.CdnUrl + key
}